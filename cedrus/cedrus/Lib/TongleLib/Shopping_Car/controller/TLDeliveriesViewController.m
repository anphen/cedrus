...
@interface TLDeliveriesViewController ()

@property (nonatomic,strong) NSArray *deliveries;

@end

@implementation TLDeliveriesViewController
...
-(void)setBaseDateType:(TLBaseDateType *)baseDateType
{
    ...
    self.deliveries = baseDateType.data_list;
}
...
-(void)viewWillDisappear:(BOOL)animated
{
    ...
    for (int i = 0; i<self.baseDateType.data_list.count; i++) {
        ...
        if (cell.rightselected) {
            [self.delegate changeDeliveriesWithController:self didAddress:self.baseDateType.data_list[i]];
        }
    }
}
@end
